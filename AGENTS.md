...
- This template reads `@dsplay/template-utils`'s `media` export directly (module-level `const { ... } = media`), not `@dsplay/react-template-utils`'s hooks — it predates that library, and was left as-is rather than force a hooks rewrite (same call made on `template-horizontal-info-bar`).
